...
mod tests {
    ...
    use crate::mc::utils::command_builder::{
        CommandAssetsConfig, CommandRamConfig, CommandResourcesConfig, CommandUserConfig, RunType,
    };
    ...
    fn main() {
        ...
        if let Some(version_index) = version_index {
            ...
            if !Path::new(format!("versions/{}", versions.clone().id).as_str()).exists() {
                fs::create_dir(format!("versions/{}", &versions.clone().id))
                    .expect("Cannot create versions dir")
            }
            let version = &versions.save_and_load(
                format!(
                    "versions/{}/{}.json",
                    &versions.clone().id,
                    &versions.clone().id
